...
cd $SLURM_SUBMIT_DIR
module purge
module load gromacs/2021.2/gcc.8.4.0-cuda.10.1.243.openmpi.4.0.3
...
if test -f "$cpi_file"; then
	...
	mpirun -np 12 mdrun_mpi -ntomp 1 -nb gpu -deffnm npt -cpi npt.cpt -append
else
	echo RUNNING NPT FROM BEGINNING: cpt file doesnt exist
	mpirun -np 12 mdrun_mpi -ntomp 1 -nb gpu -deffnm npt -s npt.tpr
fi
echo Job Done
}
